...
class LRM_Settings {
    ...
    public function beg_for_review() {
        ...
        if ( lrm_is_pro() ) {
            ...
            if ( !defined("LRM_PRO_VERSION") || version_compare(LRM_PRO_VERSION, '1.13', '<') ) {
            ...
            if ( LRM_Settings::get()->setting('general/registration/user_must_confirm_email')
                    && false ===  strpos( LRM_Settings::get()->setting('mails/lost_password/body'), '{{VERIFY_ACCOUNT_URL}}' ) ) {

                echo '<div class="notice notice-error notification-notice"><p>';

                printf(
                    '"AJAX Login and Register Modal" warning: please add tag {{VERIFY_ACCOUNT_URL}} to "Registration" mail body in <a href="%s">Emails Section</a>, else user can\'t verify account and login to your site.',
        ...
        // Update notice for 1.18 > 1.20
        if ( false === strpos( LRM_Settings::get()->setting('mails/lost_password/body'), '{{PASSWORD}}' ) ) {

            echo '<div class="notice notice-error notification-notice"><p>';

            printf(
                '"AJAX Login and Register Modal" warning: please update "Lost password" mail body in <a href="%s">Emails Section</a> (replace {{PASSWORD}} with {{CHANGE_PASSWORD_URL}})',
                admin_url('options-general.php?page=login-and-register-popup&section=mails')
            );
